...
var parser = (function () {
  var helper = {
    ...
    parse: function (data) {
      ...
      console.debug(lines);
      while (lines[0].indexOf('Seed') < 0) lines = lines.slice(1);
      var seed = /Seed:\s+([0-9]+)/.exec(lines[0]);
      var pickups = /^Excluded pickups:\s(([0-9]+\s*)*|None|none)$/.exec(lines[1]);
      ...
      for (var i = 2; i < lines.length; i++) {
        ...
        if (lineP1 && p1zones.filter(function(v) {return v == lineP1[1]}).length > 0) {
          var itemName = lineP1[3].trim().replace(/^(.+) [0-9]+$/gm, '$1');
